...
Save_DETransE_file_path = '../train_res/DETransE'
...
class DETransE(nn.Module):
    def __init__(self):
        ...
        # self.alpha_emb_1 = nn.Parameter(torch.FloatTensor(self.entity_num, int(self.emb_dimension * temporal_rate)))
        # self.alpha_emb_2 = nn.Parameter(torch.FloatTensor(self.entity_num, self.emb_dimension - int(self.emb_dimension * temporal_rate)))
        # nn.init.uniform_(self.alpha_emb_1, -6/math.sqrt(self.emb_dimension), 6/math.sqrt(self.emb_dimension))
        # nn.init.uniform_(self.alpha_emb_2, -6/math.sqrt(self.emb_dimension), 6/math.sqrt(self.emb_dimension))
        self.alpha_emb = nn.Parameter(torch.FloatTensor(self.entity_num, self.emb_dimension))
        nn.init.uniform_(self.alpha_emb, -6/math.sqrt(self.emb_dimension), 6/math.sqrt(self.emb_dimension))
        self.alpha_emb.data = F.normalize(self.alpha_emb, p=2, dim=1)

        self.w_emb = nn.Parameter(torch.FloatTensor(self.entity_num, int(self.emb_dimension * temporal_rate)))
        nn.init.uniform_(self.w_emb, -6/math.sqrt(self.emb_dimension * temporal_rate), 6/math.sqrt(self.emb_dimension * temporal_rate))
        self.w_emb.data = F.normalize(self.w_emb, p=2, dim=1)

        self.b_emb = nn.Parameter(torch.FloatTensor(self.entity_num, int(self.emb_dimension * temporal_rate)))
        nn.init.uniform_(self.b_emb, -6/math.sqrt(self.emb_dimension * temporal_rate), 6/math.sqrt(self.emb_dimension * temporal_rate))
        self.b_emb.data = F.normalize(self.b_emb, p=2, dim=1)
    ...
    def forward(self, batch, corrupt_batch):
        ...
        batch_h_emb = DEEMB(batch_h, batch_tt)
        batch_r_emb = self.relation_emb[batch_r]
        batch_t_emb = DEEMB(batch_t, batch_tt)
        c_batch_h_emb = DEEMB(c_batch_h, c_batch_tt)
        c_batch_r_emb = self.relation_emb[c_batch_r]
        c_batch_t_emb = DEEMB(c_batch_t, c_batch_tt)

        correct = []
        if L2norm:
            correct = torch.sum((batch_h_emb + batch_r_emb - batch_t_emb) ** 2, 1)
        else:
            correct = torch.norm(batch_h_emb + batch_r_emb - batch_t_emb, p=1, dim=1)
        corrupt = []
        if L2norm:
            corrupt = torch.sum((c_batch_h_emb + c_batch_r_emb - c_batch_t_emb) ** 2, 1)
        else:
            corrupt = torch.norm(c_batch_h_emb + c_batch_r_emb - c_batch_t_emb, p=1, dim=1)
...
def DEEMB(batch_entity, batch_tt):
    d = int(model.emb_dimension * temporal_rate)
    batch_tt = torch.tensor(batch_tt).reshape(len(batch_tt),1)
    batch_entity_emb = torch.cat((model.alpha_emb[batch_entity,:d] * torch.sin(model.w_emb[batch_entity] * batch_tt + model.b_emb[batch_entity]), 
                                    model.alpha_emb[batch_entity,d:]), dim=1)
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if t not in Entity2id.keys():
                ...
                entity_cnt += 1
            if tt not in Time2id.keys():
                Time2id[tt] = time_cnt
                time_cnt += 1
            Triple_all.append([Entity2id[h], Relation2id[r], Entity2id[t], Time2id[tt]])
            Triple_all_map[(Entity2id[h], Relation2id[r], Entity2id[t], Time2id[tt])] = True
...
def train():
    ...
    for iter_idx in range(Iter_num):
        ...
        for batch in batch_list:
            ...
            model.alpha_emb.data = F.normalize(model.alpha_emb, p=2, dim=1)
            model.w_emb.data = F.normalize(model.w_emb, p=2, dim=1)
            model.b_emb.data = F.normalize(model.b_emb, p=2, dim=1)
    ...
    file_path = Save_DETransE_file_path + ('_logistic_loss.pkl' if L1 else '_margin_loss.pkl')
...
def loadTestData(file_path):
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        for line in f:
            h, r, t, tt = line.split('\t')
            if h not in Entity2id.keys() or r not in Relation2id.keys() or t not in Entity2id.keys() or tt not in Time2id.keys():
               continue
            triple_all.append([Entity2id[h], Relation2id[r], Entity2id[t], Time2id[tt]])
            triple_all_map[(Entity2id[h], Relation2id[r], Entity2id[t], Time2id[tt])] = True
...
def evaluate(test_triple_all):
    ...
    h_list_emb = DEEMB(h_list, tt_list)
    r_list_emb = model.relation_emb[r_list]
    t_list_emb =  DEEMB(t_list, tt_list)
    ...
    if L2norm:
        for i in range(len(h_list)):
            ...
            dist[i] = pairwise_distances(to_compare[i].detach().reshape(1,-1), DEEMB(temp_h_list, temp_tt_list).detach(), metric='euclidean')
    else:
        for i in range(len(h_list)):
            temp_tt_list = [tt_list[i]] * model.entity_num
            dist[i] = pairwise_distances(to_compare[i].detach().reshape(1,-1), DEEMB(temp_h_list, temp_tt_list).detach(), metric='manhattan')
